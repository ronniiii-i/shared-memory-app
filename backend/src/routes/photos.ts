...
router.delete('/:id', requireAuth, async (req, res) => {
  ...
  const membership = await db.query.albumMembers.findFirst({
    where: and(eq(albumMembers.albumId, photo.albumId), eq(albumMembers.userId, userId)),
  });

  if (!isUploader && (!membership || membership.role !== 'admin')) {
    res.status(403).json({ error: 'Only the uploader or an album admin can delete this photo' });
    return;
  }
